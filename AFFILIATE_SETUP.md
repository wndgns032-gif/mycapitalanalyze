...
| AdSense | **이미 활성** (`ca-pub-9243770518153989`) |
...
**AdSense는 이미 붙어 있다** (`ca-pub-9243770518153989`, 전 페이지 확인됨, 라이브 정상).
즉 바닥 수익(노출당 수익)은 이미 확보된 상태다.

→ 따라서 제휴는 **"AdSense 위에 얹는 상방 옵션"**이다. AdSense를 대체하는 게 아니다.

- 신규 도메인에서 제휴는 전환율 0.5~2%를 요구한다. 초기엔 월 수 건 이하일 수 있다.
- **기대치 관리**: 제휴를 달았다고 수익이 바로 나지 않는다. "클릭 100회당 $17.5"(TradingView 공표 EPC) 기준으로,
  포스트 하단 박스 클릭률을 1%로 잡으면 **PV 1만당 약 $1.75** 수준이다. AdSense가 여전히 주력이다.
- 제휴가 AdSense보다 유리해지는 시점은 **개별 종목·브로커 비교형 상시 글이 쌓인 뒤**다.
  지금은 거시경제 뉴스가 주력이라 구매 의도가 낮다.
